...
if( !empty($_REQUEST['field']['Email']['required']['Email']) and isEmail($_REQUEST['field']['Email']['required']['Email'])){

    $email = $_REQUEST['field']['Email']['required']['Email'];
    $link = mysql_connect('localhost', 'geteasyf_support', 'Support1985007');
    if (!$link) {
        //die('Could not connect: ' . mysql_error());
        $result = array('success' => 0, 'msg' =>'db connection error');
        ...
        if (!$db_selected) {
           // die ('Can\'t use Data Base : ' . mysql_error());
            $result = array('success' => 0, 'msg' =>'db table error');
            echo json_encode($result);
            exit;
        }else{
            ...
            if ($num_results == 0){
                ...
                $message = !empty($_REQUEST['field']['Message']['optional']['Message']) ? $_REQUEST['field']['Message']['optional']['Message'] : 'NEWS LETTER' ;
...
$name = !empty($_REQUEST['field']['Name']['required']['Name']) ? $_REQUEST['field']['Name']['required']['Name'] : 'CUSTOMER' ;
$phone    = '000000000';
$comments = !empty($_REQUEST['field']['Message']['optional']['Message']) ? $_REQUEST['field']['Message']['optional']['Message'] : 'NO MESSAGE' ;
